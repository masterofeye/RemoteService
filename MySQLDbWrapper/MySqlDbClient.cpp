...
namespace RW{
	namespace MYSQL
	{
		MySqlDbClient* MySqlDbClient::m_Instance = nullptr;
		std::shared_ptr<spdlog::logger> MySqlDbClient::m_Logger;
		sql::Driver *MySqlDbClient::m_Driver = nullptr;
		std::unique_ptr<sql::Connection> MySqlDbClient::m_Con = nullptr;
		std::once_flag MySqlDbClient::m_once;
		...
		/******************************************************************************************************************
		@autor Ivo Kunadt
		@brief Initialisiert die MySQL Schnittstelle
		@remarks Die Parmeter für den Verbindungsaufbau sind in der Konfigurationsdatei hinterlegt und werden bei der Installation darin eingetragen.
		@param URL Protokoll, IP und Port des MySQL Datenbank Verbindung (z.b. tcp://127.0.0.1:3306)
		@param User Username für die MySQL Verbindung 
		@param Password Password für die MYSQL Verbindung
		@param Logger Logger Instanz
		@return Gibt True im Fall dessen zurück, das MySQL richtig initialisiert wurde.
		@todo Sollte eventuell verallgemeinert werden in Bezug auf MongoWrapper
		********************************************************************************************************************/
		bool MySqlDbClient::InitMySQL(std::shared_ptr<spdlog::logger> Logger, const std::string URL, const std::string &User, const std::string &Password)
		{
			try{
				m_Logger = Logger;
				...
				if (!m_Driver)
				{
					Q_ASSERT_X(true, "InitMongo", "MySQL Driver aren't available.");
					//TODO Service stoppen
					//TODO Meldung an Error Manager
					return false;
				}

				m_Con = std::unique_ptr<sql::Connection>(m_Driver->connect(URL, User, Password));
				if (!m_Con)
				{
					Q_ASSERT_X(true, "InitMongo", "Can't astablish a connection to MySQL Server.");
					//TODO Service stoppen
					//TODO Meldung an Error Manager
					return false;
				}
		...
		bool MySqlDbClient::CreateDatabase()
		{
			sql::Statement *stmt = m_Con->createStatement();
			if (!stmt->execute("CREATE DATABASE IF NOT EXIST" + DATABASE_NAME + ";"))
			{
				m_Logger->error("Can't create Database");
			}

			m_Con->setSchema(DATABASE_NAME);
		...
		void MySqlDbClient::InsertData(CollectionType CollectionType)
		{
		...
		void MySqlDbClient::InsertNewRemoteWorkstation()
		{
		...
		void MySqlDbClient::InsertNewUser()
		{
		...
		void MySqlDbClient::InsertNewGeneralInformation()
		{
		...
		void MySqlDbClient::InsertNewBackendPc()
		{
		...
		void MySqlDbClient::InsertNewSimulationPC()
		{
		...
		void  MySqlDbClient::InsertLog(BaseDbObject *Data)
		{
		...
		void MySqlDbClient::UpdateData(const std::string Collection )
		{
		...
		BaseDbObject* MySqlDbClient::GeneralInformation(QString Version)
		{
